...
@section('content')
    <div class="mb-8">
        ...
        <p class="text-gray-500 mt-1">Your conversations with your property manager.</p>
    </div>
                ...
                <a href="{{ route('tenant.messages.show', $conversation) }}"
                   class="flex items-center gap-4 px-6 py-4 border-b last:border-0 hover:bg-gray-50 transition">
                    ...
                    <div class="w-10 h-10 rounded-full bg-indigo-100 text-indigo-600 flex items-center justify-center font-semibold text-sm flex-shrink-0">
                        {{ strtoupper(substr($conversation->propertyManager->first_name, 0, 1)) }}{{ strtoupper(substr($conversation->propertyManager->last_name, 0, 1)) }}
                    </div>
